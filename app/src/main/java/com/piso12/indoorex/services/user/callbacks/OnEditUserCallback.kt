...
class OnEditUserCallback(delegate: ServiceCallback<UserDto>) : Callback<ResponseBody> {
    ...
    override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
        handleEditUserSuccess(response)
    }

    private fun handleEditUserSuccess(response: Response<ResponseBody>) {
        try {
            if (response.isSuccessful) {
                val bodyString = response.body()?.string()!!
                val user = com.piso12.indoorex.helpers.JsonParser<UserDto>()
